  ...
  return (
    <div className="flex items-center justify-between py-3 border-b border-gray-700 last:border-0">
      <div>
        ...
        <p className="text-gray-400 text-xs">{product.category_name || 'No category'}</p>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div>
        <h2 className="text-2xl font-bold text-white">
          Good {new Date().getHours() < 12 ? 'morning' : 
                new Date().getHours() < 17 ? 'afternoon' : 'evening'}, {user?.name}! 👋
        </h2>
        <p className="text-gray-400 mt-1">
          Here's what's happening today — {new Date().toLocaleDateString('en-GB', {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          color={lowStockProducts.length > 0 ? "text-yellow-400" : "text-green-400"}
          icon="⚠️"
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="bg-blue-400/20 text-blue-400 text-xs 
                             font-medium px-2 py-1 rounded-full">
              Today
            </span>
          </div>
